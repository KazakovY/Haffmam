...
let n = new Node(tree[low1].letter + tree[low2].letter, 
					tree[low1].freq + tree[low2].freq, 
					false, children, '');
					tree.push(n);
...
for (i in tree[0].letter) {
	let surchs = findChild(tree[0], tree[0].letter[i])
	let reverse = '';
	
	for (let i = surchs.length-1; i >= 0;i--){
		reverse += surchs[i];
	}

	codeDict[tree[0].letter[i]] = reverse;
}
...
function decode(){
let codeLine = result.split(" ");
let dicodedLine = '';

	for (i in codeLine){
		for (j in codeDict){
			if (codeDict[j] == codeLine[i]){
				dicodedLine += j;	
			}
		}
	}
